	    ...
	    for (int i = 0; i < recv_len; i++)
	    {
	    	...
	    	if (i % 2 == 1)
        	{
                ...
                if (enable_vid_loop)
                {
                    ...
                    // indsat - regin test
                    if (sendto(sock_video_loop, smallBuff, 2, 0, (struct sockaddr*) &si_other, slen) == -1){}
                    // indsat slut
                }
        	}
	    }
...
int main(int argc, char const *argv[])
{
    ...
    addressVidRec.sin_family = AF_INET;
    addressVidRec.sin_addr.s_addr = INADDR_ANY; // THIS IS ADDRESS
    addressVidRec.sin_port = htons( 8889 );
