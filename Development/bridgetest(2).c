...
// Declarations
    ...
    struct sockaddr_in addressVidLoop;//, si_other;
    ...
    while (runVideo1)
    {
	    ...
	    for (int i = 0; i < recv_len; i++)
	    {
		    ...
		    printf("%0x%0x" , smallBuff[0],smallBuff[1]);
                    ...
                    //if (sendto(sock_video_loop, smallBuff, 2, 0, (struct  sockaddr*) &addressVidLoop, sizeof(addressVidLoop)) == -1){}
//if (sendto(sock_video_loop, smallBuff, 2, 0, (struct addressVidRec*) &si_other, slen) == -1){}
                    // indsat slut
                }
...
int main(int argc, char const *argv[])
{
    ...
    // Socket-port 8889
    /*if (setsockopt(sock_video_loop, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT,
                                                  &opt, sizeof(opt)))
    {
        perror("setsockopt");
        exit(EXIT_FAILURE);
...
*/
    ...
    /*
    if (bind(sock_video_loop, (SA*)&addressVidLoop,
                                 sizeof(addressVidLoop))<0)
    {
        perror("bind failed");
        exit(EXIT_FAILURE);
    }
